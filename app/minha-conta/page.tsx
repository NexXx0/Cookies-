  ...
  useEffect(() => {
    const loadOrders = async () => {
      try {
        ...
        if (!res.ok) {
          if (res.status === 401) {
            setOrdersMessage("Faça login para ver seus pedidos.");
            return;
          }
          const text = await res.text();
          setOrdersMessage(text || "Não foi possível carregar os pedidos.");
          return;
        }
  ...
  const verifyEmail = async () => {
    ...
    try {
      const res = await fetch("/api/auth/verify-email-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: codeEmail, code: verifyCode }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setVerifyError((data as { error?: string })?.error || "Código inválido.");
        return;
      }
  ...
  const resetPassword = async () => {
    ...
    try {
      const res = await fetch("/api/auth/reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: codeEmail, code: resetCode, newPassword }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setResetError((data as { error?: string })?.error || "Não foi possível redefinir agora.");
        return;
      }
    ...
    <div className="account-page">
      ...
      <section className="account-grid">
        ...
        <article className="account-card">
          ...
          <p className="account-sub">Integre cada código enviado para confirmar sua conta.</p>
